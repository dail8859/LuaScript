...
void ConsoleDialog::createOutputWindow(HWND hParentWindow)
{
	...
	m_sciOutput.Call(SCI_SETUNDOCOLLECTION, 0);
	m_sciOutput.Call(SCI_SETREADONLY, 1);
	m_sciOutput.Call(SCI_USEPOPUP, 0);
	m_sciOutput.Call(SCI_SETLEXER, SCLEX_NULL);

	// Set up the styles
	setStyles(m_sciOutput);

	// Also add any additional styles
	m_sciOutput.Call(SCI_STYLESETFORE, 39, 0x0000FF);
...
void ConsoleDialog::createInputWindow(HWND hParentWindow) {
	...
	m_sciInput.Call(SCI_SETLEXER, SCLEX_LUA);
	...
	m_sciInput.Call(SCI_AUTOCSETIGNORECASE, true);
	m_sciInput.Call(SCI_AUTOCSETMAXHEIGHT, 8);
	m_sciInput.Call(SCI_AUTOCSETCANCELATSTART, false);
...
void ConsoleDialog::setStyles(GUI::ScintillaWindow &sw) {
	sw.Call(SCI_SETEOLMODE, SC_EOL_CRLF, 0);

	sw.Call(SCI_STYLESETFORE, STYLE_DEFAULT, 0x000000);
	sw.Call(SCI_STYLESETBACK, STYLE_DEFAULT, 0xFFFFFF);
	sw.Call(SCI_STYLECLEARALL, 0, 0);

	// Setup the margins
	sw.Call(SCI_SETMARGINWIDTHN, 0, 0);
	sw.Call(SCI_SETMARGINWIDTHN, 2, 0);
	sw.Call(SCI_SETMARGINWIDTHN, 3, 0);
	sw.Call(SCI_SETMARGINWIDTHN, 4, 0);

	// Margin for the prompt
	sw.Call(SCI_SETMARGINWIDTHN, 1, sw.CallString(SCI_TEXTWIDTH, STYLE_DEFAULT, ">") * 2);
	sw.Call(SCI_SETMARGINTYPEN, 1, SC_MARGIN_RTEXT);
	sw.Call(SCI_STYLESETBOLD, STYLE_LINENUMBER, true);

	sw.Call(SCI_SETMARGINLEFT, 0, 2);

	sw.Call(SCI_SETCODEPAGE, CP_UTF8);
	sw.Call(SCI_STYLESETFORE, SCE_LUA_COMMENT, 0x008000);
	sw.Call(SCI_STYLESETFORE, SCE_LUA_COMMENTLINE, 0x008000);
	sw.Call(SCI_STYLESETFORE, SCE_LUA_COMMENTDOC, 0x808000);
	sw.Call(SCI_STYLESETFORE, SCE_LUA_LITERALSTRING, 0x4A0095);
	sw.Call(SCI_STYLESETFORE, SCE_LUA_PREPROCESSOR, 0x004080);
	sw.Call(SCI_STYLESETFORE, SCE_LUA_WORD, 0xFF0000);
	sw.Call(SCI_STYLESETBOLD, SCE_LUA_WORD, 1); // keywordClass="instre1"
	sw.CallString(SCI_SETKEYWORDS, 0, "and break do else elseif end false for function goto if in local nil not or repeat return then true until while");
	sw.Call(SCI_STYLESETFORE, SCE_LUA_NUMBER, 0x0080FF);
	sw.Call(SCI_STYLESETFORE, SCE_LUA_STRING, 0x808080);
	sw.Call(SCI_STYLESETFORE, SCE_LUA_CHARACTER, 0x808080);
	sw.Call(SCI_STYLESETFORE, SCE_LUA_OPERATOR, 0x800000);
	sw.Call(SCI_STYLESETBOLD, SCE_LUA_OPERATOR, 1);
	sw.Call(SCI_STYLESETFORE, SCE_LUA_WORD2, 0xC08000);
	sw.Call(SCI_STYLESETBOLD, SCE_LUA_WORD2, 1); // keywordClass="instre2"
	sw.CallString(SCI_SETKEYWORDS, 1, "_ENV _G _VERSION assert collectgarbage dofile error getfenv getmetatable ipairs load loadfile loadstring module next pairs pcall print rawequal rawget rawlen rawset require select setfenv setmetatable tonumber tostring type unpack xpcall string table math bit32 coroutine io os debug package __index __newindex __call __add __sub __mul __div __mod __pow __unm __concat __len __eq __lt __le __gc __mode");
	sw.Call(SCI_STYLESETFORE, SCE_LUA_WORD3, 0xFF0080);
	sw.Call(SCI_STYLESETBOLD, SCE_LUA_WORD3, 1); // keywordClass="type1"
	...
	sw.Call(SCI_STYLESETFORE, SCE_LUA_WORD4, 0xA00000);
	sw.Call(SCI_STYLESETBOLD, SCE_LUA_WORD4, 1);
	sw.Call(SCI_STYLESETITALIC, SCE_LUA_WORD4, 1); // keywordClass="type2"
	...
	sw.Call(SCI_STYLESETFORE, SCE_LUA_LABEL, 0x008080);
	sw.Call(SCI_STYLESETBOLD, SCE_LUA_LABEL, 1);
}
